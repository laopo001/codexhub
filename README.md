...
CLI 可以直接启动对话，或连接指定的 CodexHub 后端：
...
ntfy hook 会为每个 `threadId + turnId` 建立一个稳定的 sequence ID，并依次更新同一条通知：`运行中`、`等待输入`、流式活动更新、`已完成`、`失败` 或 `已停止`。如果当前 Turn 有结构化 Plan，运行中消息还会按当前进行中（或待处理）步骤显示 `进度 N/M`，例如第二项进行中时显示 `进度 2/4`；全部完成时显示 `All steps complete · N/M`，例如 `All steps complete · 4/4`。没有有效步骤时只显示活动标题和已用时间，不会伪造进度。`CODEX_HUB_NTFY_UPDATE_INTERVAL_MS` 用于合并单个 Turn 的高频 token/item 事件；`CODEX_HUB_NTFY_REQUEST_INTERVAL_MS` 默认在所有 Turn 之间全局保留 5 秒请求间隔，以匹配 ntfy 默认 request bucket 的恢复速率。待发送的运行中更新会按 Turn 合并，终态会优先于它们发送；终态首次成功后会在收到终态起 60 秒内每 10 秒用同一 sequence 和 payload 重发，帮助手机补收通知。若重复发布因全局节流排到窗口外会丢弃；终态请求遇到超时、HTTP 429 或临时 5xx 时仍按原退避规则重试，失败恢复可以超过一分钟，永久 4xx 不会反复发送。当前 `ThreadSummary.source` 严格为 `cli` 时，ntfy 标题会加 `[cli]`；其它已知来源或缺少来源时不加标签。
...
`/api/projects/open` 是显式 project path thread bootstrap/persistence 入口：它返回 `machineId` 和创建/恢复的 `threadId`，但 project 本身不拥有 runtime lifecycle。machine transport 连接后 runtime 已通过 app-server 协议建立；`/api/machines/:machineId/runtime/ensure` 只作为幂等 readiness fallback，不再是正常 Add Thread 的冷启动入口。CodexHub 不提供 per-project runtime stop/restart API；runtime 不由 watcher idle 或 project delete 结束，只随 machine/server 生命周期断开或由内部 shutdown 清理。
