...
import type { VscodeSurfaceHub } from "../core/vscodeSurfaceHub.js";
import {
  vscodeSurfaceHeartbeatSchema,
  vscodeSurfaceRegistrationSchema,
  type VscodeSurfacePayload
} from "../shared/apiContract.js";

export type VscodeSurfaceRoutesContext = {
  enabled: boolean;
  protocolVersion: number;
  machines: MachineHub;
  surfaces: VscodeSurfaceHub;
};

export const registerVscodeSurfaceRoutes = (app: FastifyInstance, ctx: VscodeSurfaceRoutesContext) => {
  app.post("/api/vscode/surfaces", async (request, reply) => {
    if (!ctx.enabled) {
      reply.code(409);
      return { error: "This CodexHub server is not a VSCode authority service." };
    }
    const input = vscodeSurfaceRegistrationSchema.parse(request.body);
    if (input.protocolVersion !== ctx.protocolVersion) {
      reply.code(409);
      return { error: `Unsupported VSCode surface protocol: ${input.protocolVersion}.` };
    }
    ...
    try {
      ...
      if (
        ...
        return { ok: true, surface: ctx.surfaces.touch(input.surfaceId, input.leaseId) ?? undefined } satisfies VscodeSurfacePayload;
      }
      ...
      return { ok: true, surface } satisfies VscodeSurfacePayload;
    } catch (error) {
      reply.code(409);
  ...
  app.post("/api/vscode/surfaces/:surfaceId/heartbeat", async (request, reply) => {
    if (!ctx.enabled) {
      reply.code(409);
      return { error: "This CodexHub server is not a VSCode authority service." };
    }
    const params = z.object({ surfaceId: z.string().trim().min(1).max(200) }).parse(request.params);
    const input = vscodeSurfaceHeartbeatSchema.parse(request.body);
    if (input.protocolVersion !== ctx.protocolVersion) {
      reply.code(409);
      return { error: `Unsupported VSCode surface protocol: ${input.protocolVersion}.` };
    }
    ...
    if (!surface) {
      reply.code(404);
      return { error: "VSCode surface lease was not found." };
    }
    return { ok: true, surface } satisfies VscodeSurfacePayload;
  });

  app.delete("/api/vscode/surfaces/:surfaceId/:leaseId", async (request, reply) => {
    if (!ctx.enabled) {
      reply.code(409);
      return { error: "This CodexHub server is not a VSCode authority service." };
    }
    ...
    return { ok: true } satisfies VscodeSurfacePayload;
  });
};
