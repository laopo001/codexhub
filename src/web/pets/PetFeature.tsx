...
export const PetVisual = ({ pet, status, compact = false, dragDirection = null, lookCell = null }: PetVisualProps) => {
...
export const PetOverlay = ({ controller, onOpenThread }: PetOverlayProps) => {
  ...
  React.useEffect(() => {
    ...
    const handlePointerMove = (event: PointerEvent) => updateLookForPointer(event.clientX, event.clientY);
    window.addEventListener("pointermove", handlePointerMove, { passive: true });
    return () => window.removeEventListener("pointermove", handlePointerMove);
  }, [controller.enabled, controller.selectedPet.spriteVersionNumber, controller.status, updateLookForPointer]);
  ...
  const finishDrag = (event: React.PointerEvent<HTMLButtonElement>, cancelled = false) => {
    ...
    if (pointer) window.requestAnimationFrame(() => updateLookForPointer(pointer.x, pointer.y));
  };
