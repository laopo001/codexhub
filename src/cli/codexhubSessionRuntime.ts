...
const builtinCommandPaletteEntries = (config: JsonRecord | null): CommandPaletteEntry[] => {
...
const commandPaletteEntryFromSkill = (
...
const commandPaletteEntryFromPlugin = (summary: JsonRecord | null, fallbackName: string): CommandPaletteEntry | null => {
...
const dedupeCommandPaletteEntries = (entries: CommandPaletteEntry[]) => {
...
const pluginNameFromId = (id?: string) => id?.split("@")[0] || "";

const pluginNameFromSkillName = (name: string) => {
  const index = name.indexOf(":");
  return index > 0 ? [name.slice(0, index)] : [];
};

const stringField = (record: JsonRecord | null | undefined, key: string) => {
