...
import type { MachinesPayload, ProjectsPayload } from "../src/shared/apiContract.js";
...
test("embedded workspace startup registers projects after the machine runtime connects", async () => {
  ...
  try {
    ...
    const machines = await waitForRuntime(serverUrl);
...
const waitForRuntime = async (serverUrl: string) => {
  for (let attempt = 0; attempt < 40; attempt += 1) {
    const machines = await getJson<MachinesPayload>(`${serverUrl}/api/machines`);
