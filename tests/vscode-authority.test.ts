...
import type { HealthPayload, MachinesPayload, ProjectsPayload } from "../src/shared/apiContract.js";
...
test("one embedded authority accepts VSCode and Electron surfaces after runtime startup", async () => {
  ...
  try {
    ...
    const machines = await waitForRuntime(client);
    assert.ok(machines.machines?.some((machine) => machine.runtime?.online && machine.runtime.cliVersion));
  } finally {
    await server.stop();
...
const waitForRuntime = async (client: Client) => {
  for (let attempt = 0; attempt < 40; attempt += 1) {
    const machines = await client.request<MachinesPayload>("/api/machines");
    if (machines.machines?.some((machine) => machine.runtime?.online)) return machines;
    await new Promise((resolve) => setTimeout(resolve, 250));
  }
