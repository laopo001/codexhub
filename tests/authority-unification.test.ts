...
const assertCliFirstReuse = async () => {
  ...
  try {
    ...
    const machines = await fetchJson<{
      machines?: Array<{ machineId: string; type: string; online: boolean; runtime?: { machineId: string; online: boolean } | null }>
    }>(fixture.url, fixture.authToken, "/api/machines");
    const onlineMachines = (machines.machines ?? []).filter((machine) => machine.type === "local" && machine.online);
    const onlineRuntimes = (machines.machines ?? [])
      .map((machine) => machine.runtime)
      .filter((runtime): runtime is { machineId: string; online: boolean } => Boolean(runtime?.online));
