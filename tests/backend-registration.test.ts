...
} from "./support/backendRegistrationFixture.js";
...
const readRuntimes = async (baseUrl: string, authToken: string) =>
  (await apiJson<RuntimesBody>(baseUrl, "/api/machines", authToken)).body.machines
    ?.flatMap((machine) => machine.runtime ? [machine.runtime] : []) ?? [];
...
test("backend registration reuses child runtime and survives parent restart or cancellation", { timeout: 35_000 }, async () => {
  ...
  try {
    const localRuntime = await waitFor(
      async () => (await readRuntimes(fixture.childUrl, fixture.childAuthToken))
        .find((runtime) => runtime.machineId === fixture.childLocalMachineId),
    ...
    await waitFor(
      async () => readRuntimes(fixture.parentUrl, fixture.parentAuthToken),
...
test("parent and child backends expose one thread and one turn record stream", { timeout: 35_000 }, async () => {
  ...
  try {
    await waitFor(
      async () => readRuntimes(fixture.childUrl, fixture.childAuthToken),
      (runtimes) => runtimes?.some((runtime) =>
