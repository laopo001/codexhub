...
3. VSCode extension 注册 sidebar webview，每个 VSCode 窗口根据项目/工作区名称稳定生成 `20000-29999` 范围内的嵌入 server 起始端口，端口占用时依次 `+1`。扩展不使用外部传入的 `CODEX_HUB_PORT` 选端口，而是把当前候选端口作为显式 server `port` 参数传入，与 CLI `--port` 具有相同优先级并覆盖环境变量；顺延时必须用新的显式参数重试。`CODEX_HUB_DATA_DIR` 语义收敛到 VSCode extension `globalStorageUri` 对应的共享数据目录。每个 workspace 必须在 `workspaceState` 保存独立 parent registration machineId，并通过 embedded `parentRegistrationIdentity` 覆盖共享 profile，显示名应包含 workspace 名称。Webview iframe 和 Open in Browser 必须通过 `vscode.env.asExternalUri` 暴露 server URL，不能直接写 raw loopback URL。
