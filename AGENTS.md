...
7. VSCode 和 Electron 都调用 `src/server/embedded.ts` 复用同一套 server/Web。VSCode 不再按窗口启动 server：同一个执行 authority 只运行一个 detached authority service，Windows、macOS 和普通 Linux 固定监听 `127.0.0.1:28788`，WSL 固定使用 `127.0.0.1:28789`，避免 mirrored networking 下与 Windows 的 `28788` 冲突；Windows、每个 WSL distro、每个 Remote SSH host/user 和容器仍是彼此独立的 authority。固定端口被非匹配服务占用时必须明确失败，不能顺延或读取 `CODEX_HUB_PORT`。authority 只监听 loopback，默认不启用 access token；只有 extension host 环境或 authority `config.yaml` 的 `env.CODEX_HUB_AUTH_TOKEN` 显式非空时才启用认证。每个 VSCode 窗口通过 lease 注册自己的 file workspace folders，所有窗口共享 authority 的 `globalStorageUri` 数据目录、`config.yaml`、唯一 local runtime 和 parent machine transport；窗口 surface 和自动 workspace projects 只作为 transient 内存投影，不写入持久配置。Electron 默认随机端口，只有显式 `CODEX_HUB_PORT` 时才固定端口。
...
4. VSCode surfaces：`POST /api/vscode/surfaces`、`POST /api/vscode/surfaces/:surfaceId/heartbeat`、`DELETE /api/vscode/surfaces/:surfaceId/:leaseId`。只在 VSCode authority service 启用；注册要验证当前 authority 的 local launcher 和每个 workspace path，合并所有有效 lease 的 transient projects，但不能启动 runtime/thread。旧 lease 的 heartbeat/unregister 不能删除同一 surface 的新 lease。
...
3. VSCode extension 注册 sidebar webview，但 server 生命周期属于执行 authority，不属于窗口。extension 先 probe 固定 loopback 端口：Windows、macOS、普通 Linux 为 `28788`，WSL 为 `28789`；没有匹配 `authorityId` 和 surface protocol 的服务时，才从 VSIX 中 detached 启动 `authority-service.cjs`。authority ID 保存在 `globalStorageUri/vscode-authority-id`，`config.yaml` 和 service log 也属于同一 authority 数据目录。authority 仅监听 `127.0.0.1` 且默认不启用认证，也不能自动生成 token；只有 extension host 环境或 authority `config.yaml` 的 `env.CODEX_HUB_AUTH_TOKEN` 显式非空时才启用。显式 token 只通过子进程环境注入，不能出现在命令行或日志；认证启用后 surface API、Web API/WebSocket 和 iframe/Open in Browser URL 都必须携带它。端口不读取 `CODEX_HUB_PORT`、不顺延，被其他服务或不同 authority 占用时直接报错。Webview iframe 和 Open in Browser 必须通过 `vscode.env.asExternalUri` 暴露 server URL，不能直接写 raw loopback URL。
4. 每个 VSCode 窗口使用唯一 `surfaceId + leaseId` 调用 `/api/vscode/surfaces` 注册当前 file workspace folders，并每 10 秒 heartbeat；deactivate 主动 unregister，进程异常退出则由 30 秒 lease 清理。authority 合并所有活动窗口的 transient projects，同一路径由多个窗口引用时必须保留到最后一个 lease 消失；注册只能验证目录，不能启动 app-server/runtime/thread，真正的 runtime 冷启动留给 Add Thread。没有 file workspace folder 时只显示状态页且不保留 lease。
5. VSCode authority 的 local machine、官方 app-server runtime、SSH/tasks/integrations/Registered 配置和 parent machine transport 在同一执行环境的窗口间共享；UI 仍按 iframe URL 中的 workspace paths 过滤项目，localStorage 按稳定 workspace scope 隔离，任务完成通知不能发到不包含该 project path 的窗口。Windows host、WSL、Remote SSH 等不同 authority 不能合并成同一个 machine。
